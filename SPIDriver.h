...
public:
    ...
    void init(void);
    void setFunctional(spiBASE_t *spi, unsigned port);
    void setData(spiBASE_t *spi, unsigned group, unsigned short data[]);
    unsigned getData(spiBASE_t *spi, unsigned group, unsigned short data[]);
    void transfer(spiBASE_t *spi, unsigned group);
    int  isTransferComplete(spiBASE_t *spi, unsigned group);
    void enableGroupNotification(spiBASE_t *spi, unsigned group, unsigned level);
    void disableGroupNotification(spiBASE_t *spi, unsigned group);

    void MIBSPI1_Parity(void);
    void MIBSPI3_Parity(void);
    void MIBSPIP5_Parity(void);

    /** @fn void notification(unsigned flags)
    *   @brief Error interrupt callback
    *   @param[in] copy of error interrupt flags
    *
    ...
    */
    void notification(unsigned flags);


    /** @fn void groupNotification(unsigned group)
    *   @breif Transfer complete notification callback
    *   @param[in] Transfer group
    *
    ...
    * the interrupt.
    */
    void groupNotification(unsigned group);
